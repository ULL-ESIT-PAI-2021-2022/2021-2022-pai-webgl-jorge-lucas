/**
 * @desc This function returns a code in glsl for the Vertex Shader
 * (pre-rasterization)
 * @returns {String}
 */
const xVertexShader = () => {
...
/**
 * @desc This function returns a code in glsl for the Fragment Shader
 * (post-rasterization)
 * @returns {String}
 */
const xFragmentShader = () => {
  return `
    ...
    vec2 f(vec2 zComplexNumber, vec2 cComplexNumber) {
      return mat2(zComplexNumber, -zComplexNumber.y, zComplexNumber.x) *
      zComplexNumber + cComplexNumber;
    }
    ...
    void main() {
      ...
      for (int i = 0; i < 10000; i++) {
        ...
        iterations = i;
        zComplexNumber = f(zComplexNumber, cComplexNumber);
        if (length(zComplexNumber) > 2.0) {
          escaped = true;
          break;
        }
      }
      gl_FragColor = escaped ? vec4(palette(float(iterations) / 
      float(u_maxIterations), 
      vec3(0.0), vec3(0.59, 0.55, 0.75), 
      vec3(0.1, 0.2, 0.3), vec3(0.75)), 1.0) : vec4(vec3(0.85, 0.99, 1.0), 1.0);
    }
  `
}

/**
 * @desc This is the function that loads, compiles and represents the pixels 
 * in the canvas
 */
function main() {
  ...
  /** get shader code from the functions */
  let vertex_shader_src = xVertexShader();
  let fragment_shader_src = xFragmentShader();
  ...
  /**
   * @desc This function renders the actual frame
   */
